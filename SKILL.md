...
model-to-model messages, and machine-facing payloads. An intermediate progress
or tool-planning message must contain only one raw B72 wire line: no English
summary, duplicate explanation, label, or marker such as `B72-INTERMEDIATE:`.
...
progress channel for exactly the raw B72 line. Do not add a prose preamble or
follow-up to that message. If the host suppresses intermittent
